...
// ✅ IMPORTANT: use env vars in production
const stripeSecretKey = process.env.STRIPE_SECRET_KEY || 'REMOVED_STRIPE_LIVE_KEY';
const webhookSecret = process.env.STRIPE_WEBHOOK_SECRET || 'whsec_QcumBf3YBDiTzem2EsrBodKrhxTMDNLL';
...
export class SubscriptionController {
  ...
  async subscribe(@Req() req, @Body() body) {
    ...
    try {
      ...
      // Apple/Google: just update DB (no Stripe)
      if (subscriptionSource === 'APPLE' || subscriptionSource === 'GOOGLE') {
        ...
        else if (billingType === 'YEARLY') expiresAt.setFullYear(expiresAt.getFullYear() + 1);
  ...
  async getLeadPaymentStatus(@Query('session_id') sessionId: string) {
    if (!sessionId) throw new BadRequestException('Missing session_id query parameter');
      ...
      line_items: [
        {
          price_data: {
            currency: 'usd',
            product_data: { name: 'Finalize Sale Fee (5% Profit)' },
            unit_amount: appFee,
          },
          quantity: 1,
        },
  ...
  async getStatus(@Req() req) {
    ...
    const effectiveStatus = isExpired ? 'INACTIVE' : (subscription.status ?? 'INACTIVE');
  ...
  async checkEligibility(@Req() req, @Body('type') jobType: 'SELL' | 'REMOVE' | 'OTHER') {
    const plan = getCustomerPlanDetails(req.user.subscription?.type);

    let canPost = false;
    if (jobType === 'SELL' || jobType === 'REMOVE') canPost = canPostJob(req.user, jobType);
    else canPost = plan?.allowExtraServices || false;
  ...
  async updateSubscriptionId(
    ...
    if (customerId) updateData['subscription.customerId'] = customerId;
  ...
  async webhookHandler(
    @Req() req: Request & { body: Buffer }, // Buffer from express.raw()
    @Res() res: Response,
    ...
    try {
      ...
      if (subscriptionStatus === 'active' || subscriptionStatus === 'trialing') mappedStatus = 'ACTIVE';
      else if (subscriptionStatus === 'expired') mappedStatus = 'INACTIVE';
      else if (subscriptionStatus === 'canceled') mappedStatus = 'CANCELED';
